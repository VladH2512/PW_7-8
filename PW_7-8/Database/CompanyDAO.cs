        ...
        public override void Change()
        {
            ...
            using (MySqlConnection connection = Datebase.GetConnection())
            {
                ...
                try
                {
                    ...
                    // Проверка сохранений
                    if (list.companies[Id].ChangeHistory.History.Count != 0)
                    {
                        Console.Write("\nОтменить действия по изминению?(y) - ");
                        string select = Console.ReadLine();

                        if (select.ToLower() == "y")
                        {
                            company.RestoreCompany(list.companies[Id].ChangeHistory.History.Pop());
                            company.ChangeHistory = list.companies[Id].ChangeHistory;
                            cancel = true;
                        }
                        
                    }

                    // Проверка на не отмену действия по изминению
                    if(cancel == false)
                    {
                        company = list.companies[Id].HistoryPush(list.companies[Id].SaveChange()).SetId().SetDirectorId(null);
